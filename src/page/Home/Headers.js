import React from "react";
// import background from "../../img/"
import bg from "../../assets/img/man.jpg"
import "./Headers.css"

const Headers = () => {
    return (
        <>
            <div className="section-headers d-flex flex-column" style={{ backgroundImage: `linear-gradient(to left, rgba(255,0,0,0), rgba(255,255,255,1)), url(${bg})`, backgroundSize: "100%", height: "800px", backgroundRepeat: "no-repeat", backdropFilter: "blur(10px)" }}>
                <div className="title mx-5 mt-5">
                    <h1 style={{ fontFamily: 'Georgia', color: '#0D253C', fontWeight: '700' }}>Share Information <br /> and Educate People</h1>
                </div>
                <div className="content mx-5 mt-3">
                    <p style={{ fontFamily: 'Lato', color: "#0D253C" }}>Everyone has their point of view of something, but just <br />
                        don't be afraid to express the facts. Be an author and <br />
                        share you prespective of something to the world.
                    </p>
                </div>
                <div className="explore mx-5 mt-3">
                    <button className="btn btn-primary">Start Exploring</button>
                </div>
            </div>
        </>
    )
}

export default Headers